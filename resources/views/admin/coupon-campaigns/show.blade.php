    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            ...
            <div class="mt-4 flex gap-x-3 sm:mt-0">
                <a href="{{ route('admin.coupon-campaigns.edit', $couponCampaign) }}"
                    ...
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th scope="col"
                                ...
                                Code</th>
                            <th scope="col"
                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">Value
                            </th>
                            <th scope="col"
                            ...
                            <th scope="col"
                                ...
                                Expires</th>
                            <th scope="col"
                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">Used
                                At</th>
                            <th scope="col"
                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">
                                Redeemed By</th>
                        ...
                        @forelse($coupons as $coupon)
                            <tr x-data="{ couponId: {{ $coupon->id }} }">
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                                    <span
                                        class="font-mono text-sm font-medium text-gray-900 dark:text-white">{{ $coupon->code }}</span>
                                </td>
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm">
                                    <template x-if="redeemedCoupons[couponId]">
                                        <span
                                            class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300">Used</span>
                                    </template>
                                    <template x-if="!redeemedCoupons[couponId]">
                                        <span>
                                            @if ($coupon->status === 'unused')
                                                <span
                                                    class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300">Unused</span>
                                            @elseif($coupon->status === 'used')
                                                <span
                                                    class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300">Used</span>
                                            @elseif($coupon->status === 'expired')
                                                <span
                                                    class="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-300">Expired</span>
                                            @else
                                                <span
                                                    class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800 dark:bg-gray-600 dark:text-gray-300">Disabled</span>
                                            @endif
                                        </span>
                                    </template>
                                </td>
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    <template x-if="redeemedCoupons[couponId]">
                                        <span
                                            x-text="new Date(redeemedCoupons[couponId].used_at).toLocaleString()"></span>
                                    </template>
                                    <template x-if="!redeemedCoupons[couponId]">
                                        ...
                                        <div class="flex items-center">
                                            <div>
                                                <div class="font-medium text-gray-900 dark:text-white"
                                                    x-text="redeemedCoupons[couponId].redeemed_by.name"></div>
                                                <div class="text-gray-500 dark:text-gray-400"
                                                    x-text="redeemedCoupons[couponId].redeemed_by.email"></div>
                                            </div>
                                        </div>
                                    </template>
                                    <template x-if="!redeemedCoupons[couponId]">
                                        <span>
                                            @if ($coupon->usedByUser)
                                                <div class="flex items-center">
                                                    <div>
                                                        <div class="font-medium text-gray-900 dark:text-white">
                                                            {{ $coupon->usedByUser->name }}</div>
                                                        <div class="text-gray-500 dark:text-gray-400">
                                                            {{ $coupon->usedByUser->email }}</div>
                                                    </div>
                                                </div>
                                            @else
                                                -
                                ...
                                <td
                                    ...
                                    <a href="{{ route('admin.coupons.image.download', $coupon) }}"
                                    ...
                                    @if ($coupon->status === 'unused')
                                        <template x-if="!redeemedCoupons[couponId]">
                                            <button type="button"
                                                ...
                                                x-on:click="disableCouponId = {{ $coupon->id }}; disableCouponCode = '{{ $coupon->code }}'; $dispatch('open-modal', 'confirm-coupon-disable')">Disable</button>
                                        </template>
                                    @endif
                                    @if ($coupon->status === 'disabled')
                                        <button type="button"
                                            class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 mr-3"
                                            x-on:click="enableCouponId = {{ $coupon->id }}; enableCouponCode = '{{ $coupon->code }}'; $dispatch('open-modal', 'confirm-coupon-enable')">Enable</button>
                                    @endif
                                    <button type="button"
                                        class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300"
                                        x-on:click="deleteCouponId = {{ $coupon->id }}; deleteCouponCode = '{{ $coupon->code }}'; $dispatch('open-modal', 'confirm-coupon-deletion')">Delete</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
        ...
        <x-modal name="confirm-coupon-disable" maxWidth="md">
            <div class="p-6">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Are you sure you want to disable coupon <span class="font-mono font-semibold"
                        x-text="disableCouponCode"></span>? This coupon will no longer be redeemable.
                </p>
        ...
        <x-modal name="confirm-coupon-enable" maxWidth="md">
            <div class="p-6">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Are you sure you want to enable coupon <span class="font-mono font-semibold"
                        x-text="enableCouponCode"></span>? This coupon will be redeemable again.
                </p>
        ...
        <x-modal name="confirm-coupon-deletion" maxWidth="md">
            <div class="p-6">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Are you sure you want to delete coupon <span class="font-mono font-semibold"
                        x-text="deleteCouponCode"></span>? This action cannot be undone.
                </p>
